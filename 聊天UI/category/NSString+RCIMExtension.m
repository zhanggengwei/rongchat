...
NSString *const RCIMPhoneRegex =  @"\\d{3,4}[- ]?\\d{7,8}";

@implementation NSString (RCIMExtension)
...
- (NSArray<NSString *> *)lcck_allURLLinks {
    NSString *URLRegex =  RCIMURLRegex;
...
- (NSArray<NSString *> *)lcck_allPhoneNumbers {
    //匹配10到12位连续数字，或者带连字符/空格的固话号，空格和连字符可以省略。
    NSString *phoneRegex = RCIMPhoneRegex;
...
- (BOOL)lcck_isType:(NSTextCheckingType)type {
    switch (type) {
        case NSTextCheckingTypeLink:
            return ([self lcck_allMatchsWithPattern:RCIMURLRegex error:nil].count > 0);
            break;
            case NSTextCheckingTypePhoneNumber:
            return ([self lcck_allMatchsWithPattern:RCIMPhoneRegex error:nil].count > 0);
            break;
        default:
            break;
    }
...
- (NSString *)lcck_pathForConversationBackgroundImage {
    NSString *path = nil;
    /*= [NSString stringWithFormat:@"%@/APP/%@/User/%@/Conversation/%@/Background/", [NSFileManager lcck_documentsPath], [LCChatKit sharedInstance].appId,[RCIMSessionService sharedInstance].clientId, [RCIMConversationService sharedInstance].currentConversation.conversationId];
                    */
