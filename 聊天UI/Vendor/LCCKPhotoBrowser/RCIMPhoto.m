...
+ (RCIMPhoto *)photoWithImage:(UIImage *)image {
	return [[RCIMPhoto alloc] initWithImage:image];
}

+ (RCIMPhoto *)photoWithFilePath:(NSString *)path {
	return [[RCIMPhoto alloc] initWithFilePath:path];
}

+ (RCIMPhoto *)photoWithURL:(NSURL *)url {
	return [[RCIMPhoto alloc] initWithURL:url];
}
...
+ (NSArray *)photosWithImages:(NSArray *)imagesArray {
    ...
    for (UIImage *image in imagesArray) {
        if ([image isKindOfClass:[UIImage class]]) {
            RCIMPhoto *photo = [RCIMPhoto photoWithImage:image];
...
+ (NSArray *)photosWithFilePaths:(NSArray *)pathsArray {
    ...
    for (NSString *path in pathsArray) {
        if ([path isKindOfClass:[NSString class]]) {
            RCIMPhoto *photo = [RCIMPhoto photoWithFilePath:path];
            [photos addObject:photo];
        }
    }
...
+ (NSArray *)photosWithURLs:(NSArray *)urlsArray {
    ...
    for (id url in urlsArray) {
        if ([url isKindOfClass:[NSURL class]]) {
            RCIMPhoto *photo = [RCIMPhoto photoWithURL:url];
            [photos addObject:photo];
        }
        else if ([url isKindOfClass:[NSString class]]) {
            RCIMPhoto *photo = [RCIMPhoto photoWithURL:[NSURL URLWithString:url]];
            [photos addObject:photo];
        }
    }
...
#pragma mark RCIMPhoto Protocol Methods
...
- (void)loadImageFromFileAsync {
    @autoreleasepool {
        @try {
            ...
            if (!_underlyingImage) {
                //RCIMLog(@"Error loading photo from path: %@", _photoPath);
            }
        } @finally {
...
- (void)imageLoadingComplete {
    ...
    // Complete so notify
    _loadingInProgress = NO;
    [[NSNotificationCenter defaultCenter] postNotificationName:RCIMPhoto_LOADING_DID_END_NOTIFICATION
                                                        object:self];
}
