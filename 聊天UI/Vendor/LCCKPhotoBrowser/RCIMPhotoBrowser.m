...
NSLocalizedStringFromTableInBundle((key), nil, [NSBundle bundleWithPath:[[NSBundle bundleForClass: self.class] pathForResource:@"RCIMPBLocalizations" ofType:@"bundle"]], nil)
#endif

// Private
@interface RCIMPhotoBrowser () <RCIMZoomingScrollViewDelegate> {
	// Data
...
- (RCIMZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index;
- (RCIMZoomingScrollView *)pageDisplayingPhoto:(id<RCIMPhoto>)photo;
- (RCIMZoomingScrollView *)dequeueRecycledPage;
- (void)configurePage:(RCIMZoomingScrollView *)page forIndex:(NSUInteger)index;
...
- (CGRect)frameForCaptionView:(RCIMCaptionView *)captionView atIndex:(NSUInteger)index;
...
- (UIImage *)imageForPhoto:(id<RCIMPhoto>)photo;
- (void)loadAdjacentPhotosIfNecessary:(id<RCIMPhoto>)photo;
- (void)releaseAllUnderlyingPhotos;

@end

// RCIMPhotoBrowser
@implementation RCIMPhotoBrowser
...
#pragma mark - RCIMZoomingScrollViewDelegate
...
//        _scaleImage = nil;
        ...
        // Listen for RCIMPhoto notifications
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(handleRCIMPhotoLoadingDidEndNotification:)
                                                     name:RCIMPhoto_LOADING_DID_END_NOTIFICATION
                                                   object:nil];
    }
...
- (id)initWithPhotoURLs:(NSArray *)photoURLsArray {
    if ((self = [self init])) {
        NSArray *photosArray = [RCIMPhoto photosWithURLs:photoURLsArray];
		_photos = [[NSMutableArray alloc] initWithArray:photosArray];
	}
	return self;
}
...
- (void)dealloc {
    ...
    RCIMPBLog(@"%@, %@, %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd), [NSString stringWithFormat:@"%i", __LINE__]);
}
...
//    // Initial Setup
//    RCIMZoomingScrollView *scrollView = [self pageDisplayedAtIndex:_currentPageIndex];
//    //RCIMTapDetectingImageView *scrollView.photoImageView = scrollView.photoImageView;
//
...
//    UIImage *imageFromView = _scaleImage ? _scaleImage : [self getImageFromView:_senderViewForAnimation];
    id<RCIMPhoto> photo = [self photoAtIndex:_currentPageIndex];
...
- (void)performCloseAnimationWithScrollView:(RCIMZoomingScrollView*)scrollView {
...
//        [_doneButton setTitle:RCIMPhotoBrowserLocalizedStrings(@"Done") forState:UIControlStateNormal];
...
//    [UIImage imageNamed:@"RCIMPhotoBrowser.bundle/images/RCIMPhotoBrowser_arrowLeft.png"]          : _leftArrowImage;
//
//    UIImage *rightButtonImage = (_rightArrowImage == nil) ?
//    [UIImage imageNamed:@"RCIMPhotoBrowser.bundle/images/RCIMPhotoBrowser_arrowRight.png"]         : _rightArrowImage;
//
//    UIImage *leftButtonSelectedImage = (_leftArrowSelectedImage == nil) ?
//    [UIImage imageNamed:@"RCIMPhotoBrowser.bundle/images/RCIMPhotoBrowser_arrowLeftSelected.png"]  : _leftArrowSelectedImage;
//
//    UIImage *rightButtonSelectedImage = (_rightArrowSelectedImage == nil) ?
//    [UIImage imageNamed:@"RCIMPhotoBrowser.bundle/images/RCIMPhotoBrowser_arrowRightSelected.png"] : _rightArrowSelectedImage;
//
//    // Arrows
...
//    _doneButton.frame = [self frameForDoneButtonAtOrientation:currentOrientation];
	...
	for (RCIMZoomingScrollView *page in _visiblePages) {
...
- (RCIMCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    RCIMCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <RCIMPhoto> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[RCIMCaptionView alloc] initWithPhoto:photo];
        }
    }
...
- (void)loadAdjacentPhotosIfNecessary:(id<RCIMPhoto>)photo {
    RCIMZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <RCIMPhoto> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImageExisted]) {
                    [photo loadUnderlyingImageAndNotify];
                    RCIMPBLog(@"Pre-loading image at index %@", @(pageIndex-1));
                }
            }
...
#pragma mark - RCIMPhoto Loading Notification

- (void)handleRCIMPhotoLoadingDidEndNotification:(NSNotification *)notification {
    id <RCIMPhoto> photo = [notification object];
    RCIMZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        if ([photo underlyingImageExisted]) {
            // Display image
            ...
            RCIMPBLog(@"Success to load image");
        } else {
            // Failed to load
            [page displayImageFailure];
            RCIMPBLog(@"Failed to load image");
        }
    }
}
...
- (void)tilePages {
	...
	for (RCIMZoomingScrollView *page in _visiblePages) {
		...
		if (pageIndex < (NSUInteger)iFirstIndex || pageIndex > (NSUInteger)iLastIndex) {
			...
			RCIMPBLog(@"Removed page at index %@", @(PAGE_INDEX(page)));
		}
	}
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            // Add new page
			RCIMZoomingScrollView *page;
//            page = [[RCIMZoomingScrollView alloc] initWithPhotoBrowser:self];
            page = [[RCIMZoomingScrollView alloc] initWithPhotoDelegate:self];
			...
			RCIMPBLog(@"Added page at index %@", @(index));

            // Add caption
            RCIMCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
...
- (RCIMZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
	RCIMZoomingScrollView *thePage = nil;
	for (RCIMZoomingScrollView *page in _visiblePages) {
...
- (void)configurePage:(RCIMZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    __block __weak RCIMPhoto *photo = (RCIMPhoto *)page.photo;
    __weak RCIMZoomingScrollView* weakPage = page;
...
- (RCIMZoomingScrollView *)dequeueRecycledPage {
	RCIMZoomingScrollView *page = [_recycledPages anyObject];
	if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    id <RCIMPhoto> currentPhoto = [self photoAtIndex:index];
...
//		_counterLabel.text = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), RCIMPhotoBrowserLocalizedStrings(@"of"), (unsigned long)[self numberOfPhotos]];
//	} else {
...
//					[selfBlock hideControlsAfterDelay];
        ...
        else
        {
            ...
            self.actionsSheet.cancelButtonIndex = [self.actionsSheet addButtonWithTitle:RCIMPhotoBrowserLocalizedStrings(@"Cancel")];
