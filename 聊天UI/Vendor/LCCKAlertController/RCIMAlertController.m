...
#import "RCIMAlertController.h"
#import <objc/runtime.h>

#define PROPERTY(property) NSStringFromSelector(@selector(property))

@interface RCIMAlertAction ()
@property (nonatomic, copy) NSString *title;
@property (nonatomic, assign) RCIMAlertActionStyle style;
@property (nonatomic, copy) void (^handler)(RCIMAlertAction *action);
- (void)performAction;
@end

@implementation RCIMAlertAction

+ (instancetype)actionWithTitle:(NSString *)title style:(RCIMAlertActionStyle)style handler:(void (^)(RCIMAlertAction *action))handler {
    return [[self alloc] initWithTitle:title style:style handler:handler];
}

+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(RCIMAlertAction *action))handler {
    return [[self alloc] initWithTitle:title style:RCIMAlertActionStyleDefault handler:handler];
}

- (instancetype)initWithTitle:(NSString *)title style:(RCIMAlertActionStyle)style handler:(void (^)(RCIMAlertAction *action))handler {
    if ((self = [super init])) {
        _title = [title copy];
        _style = style;
...
@interface RCIMAlertController () <UIActionSheetDelegate, UIAlertViewDelegate> {
    struct {
        unsigned int isShowingAlert:1;
    } _flags;
}
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(RCIMAlertControllerStyle)preferredStyle NS_DESIGNATED_INITIALIZER;

@property (nonatomic, copy) NSArray *willDismissBlocks;
@property (nonatomic, copy) NSArray *didDismissBlocks;

// iOS 8
@property (nonatomic, strong) RCIMExtendedAlertController *alertController;

// Universal
@property (nonatomic, weak) RCIMAlertAction *executedAlertAction;
...
@implementation RCIMAlertController
...
+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(RCIMAlertControllerStyle)preferredStyle {
...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(RCIMAlertControllerStyle)preferredStyle {
    if ((self = [super init])) {
        ...
        if ([self alertControllerAvailable]) {
            _alertController = [RCIMExtendedAlertController alertControllerWithTitle:title message:message preferredStyle:(UIAlertControllerStyle)preferredStyle];
        } else {
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 80000
            if (preferredStyle == RCIMAlertControllerStyleActionSheet) {
...
- (void)addAction:(RCIMAlertAction *)action {
    NSAssert([action isKindOfClass:RCIMAlertAction.class], @"Must be of type RCIMAlertAction");
    ...
    if ([self alertControllerAvailable]) {
        ...
        if (self.preferredStyle == RCIMAlertControllerStyleActionSheet) {
            NSUInteger currentButtonIndex = [self.actionSheet addButtonWithTitle:action.title];

            if (action.style == RCIMAlertActionStyleDestructive) {
                self.actionSheet.destructiveButtonIndex = currentButtonIndex;
            } else if (action.style == RCIMAlertActionStyleCancel) {
            ...
            if (action.style == RCIMAlertActionStyleCancel) {
...
- (void)addTextFieldWithConfigurationHandler:(void (^)(UITextField *textField))configurationHandler {
    if ([self alertControllerAvailable]) {
        ...
        NSAssert(self.preferredStyle == RCIMAlertControllerStyleAlert, @"Text fields are only supported for alerts.");
...
- (NSArray *)textFields {
    ...
    } else if (self.preferredStyle == RCIMAlertControllerStyleAlert) {
...
static NSUInteger RCIMVisibleAlertsCount = 0;
+ (BOOL)hasVisibleAlertController {
    return RCIMVisibleAlertsCount > 0;
}
...
- (void)showWithSender:(id)sender arrowDirection:(UIPopoverArrowDirection)arrowDirection controller:(UIViewController *)controller animated:(BOOL)animated completion:(void (^)(void))completion {
    if ([self alertControllerAvailable]) {
        ...
        if (self.preferredStyle == RCIMAlertControllerStyleAlert) {
            if (!controller) {
        ...
        RCIMExtendedAlertController *alertController = self.alertController;
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    if ([self alertControllerAvailable]) {
        ...
        if (completion) {
            [self addDidDismissBlock:^(RCIMAlertAction *action) { completion(); }];
        }
        if (self.preferredStyle == RCIMAlertControllerStyleActionSheet) {
...
- (void)addWillDismissBlock:(void (^)(RCIMAlertAction *action))willDismissBlock {
...
- (void)addDidDismissBlock:(void (^)(RCIMAlertAction *action))didDismissBlock {
...
- (RCIMAlertAction *)actionForButtonIndex:(NSInteger)index {
    return index >= 0 ? self.actions[index] : nil;
}

- (void)performBlocks:(NSString *)blocksStorageName withAction:(RCIMAlertAction *)alertAction {
    // Load variable and nil out.
    NSArray *blocks = [self valueForKey:blocksStorageName];
    [self setValue:nil forKey:blocksStorageName];

    for (void (^block)(RCIMAlertAction *action) in blocks) {
        block(alertAction);
    }
}

- (void)viewWillDismissWithButtonIndex:(NSInteger)buttonIndex {
    RCIMAlertAction *action = [self actionForButtonIndex:buttonIndex];
...
@implementation RCIMAlertController (Convenience)
...
+ (instancetype)alertWithTitle:(NSString *)title message:(NSString *)message {
    return [[self alloc] initWithTitle:title message:message preferredStyle:RCIMAlertControllerStyleAlert];
}

+ (instancetype)actionSheetWithTitle:(NSString *)title {
    return [[self alloc] initWithTitle:title message:nil preferredStyle:RCIMAlertControllerStyleActionSheet];
}
...
+ (instancetype)presentDismissableAlertWithTitle:(NSString *)title message:(NSString *)message controller:(UIViewController *)controller {
    RCIMAlertController *alertController = [self alertWithTitle:title message:message];
    [alertController addAction:[RCIMAlertAction actionWithTitle:NSLocalizedString(@"Dismiss", @"") style:RCIMAlertActionStyleCancel handler:NULL]];
...
- (void)addCancelActionWithHandler:(void (^)(RCIMAlertAction *action))handler {
    [self addAction:[RCIMAlertAction actionWithTitle:NSLocalizedString(@"Cancel", @"") style:RCIMAlertActionStyleCancel handler:handler]];
}
