...
@interface RCCKChatMoreView ()<UIScrollViewDelegate>
...
@implementation RCCKChatMoreView
...
- (NSArray<Class> *)sortedInputViewPluginArray {
    ...
    NSArray *inputViewPluginDictArray = [RCInputViewPluginArray copy];
    
    NSArray<NSDictionary *> *notSortedDefaultInputViewPlugin = [inputViewPluginDictArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"(RCInputViewPluginTypeKey < 0)"]];
    NSArray<NSDictionary *> *notSortedCustomInputViewPlugin = [inputViewPluginDictArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"(RCInputViewPluginTypeKey > 0)"]];
    ...
    [notSortedDefaultInputViewPlugin enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull inputViewPluginDict, NSUInteger idx, BOOL * _Nonnull stop) {
        [notSortedDefaultInputViewPluginMutableTypes addObject:inputViewPluginDict[RCInputViewPluginTypeKey]];
    }];
    [notSortedCustomInputViewPlugin enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull inputViewPluginDict, NSUInteger idx, BOOL * _Nonnull stop) {
        [notSortedCustomInputViewPluginMutableTypes addObject:inputViewPluginDict[RCInputViewPluginTypeKey]];
    }];
    ...
    NSMutableArray *sortedInputViewPluginArray = [NSMutableArray arrayWithCapacity:[[RCInputViewPluginDict allKeys] count]];
    [sortedDefaultInputViewPluginTypes enumerateObjectsUsingBlock:^(NSNumber * _Nonnull typeKey, NSUInteger idx, BOOL * _Nonnull stop) {
        [sortedInputViewPluginArray addObject:[RCInputViewPluginDict objectForKey:typeKey]];
    }];
...
- (void)moreView:(RCCKChatMoreView *)moreView selectIndex:(NSInteger)itemType {
    NSNumber *typeKey = @(itemType);
    id<LCCKInputViewPluginDelegate> inputViewPlugin = [[RCInputViewPluginDict objectForKey:typeKey] new];
    inputViewPlugin.inputViewRef = self.inputViewRef;
    [inputViewPlugin pluginDidClicked];
}

- (NSArray *)titlesOfMoreView:(RCCKChatMoreView *)moreView {
    NSMutableArray *titles = [NSMutableArray arrayWithCapacity:[[RCInputViewPluginDict allKeys] count]];
...
- (NSArray<UIImage *> *)imagesOfMoreView:(RCCKChatMoreView *)moreView {
    NSMutableArray *images = [NSMutableArray arrayWithCapacity:[[RCInputViewPluginDict allKeys] count]];
    [self.sortedInputViewPluginArray enumerateObjectsUsingBlock:^(Class _Nonnull aClass, NSUInteger idx, BOOL * _Nonnull stop) {
        id<LCCKInputViewPluginDelegate> inputViewPlugin = [aClass new];
...
- (NSInteger)inputViewPluginTypeForItemTag:(NSInteger)tag {
    NSArray<NSNumber *> *allPlugins = [RCInputViewPluginDict allKeys];
...
- (void)setupItems {
    ...
    [self.titles enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        LCCKInputViewPlugin *item = [[[RCInputViewPluginDict objectForKey:typeKey] alloc] initWithFrame:CGRectMake(startX, startY, self.itemSize.width, self.itemSize.height)];
...
- (UIColor *)messageInputViewMorePanelBackgroundColor {
    ...
    _messageInputViewMorePanelBackgroundColor = [[RCIMSettingService shareManager] defaultThemeColorForKey:@"MessageInputView-MorePanel-BackgroundColor"];
    return _messageInputViewMorePanelBackgroundColor;
}
